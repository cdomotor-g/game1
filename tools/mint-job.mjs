 ...
 *
 *   `a100-large`, not l40sx1. Qwen-Image is 20B and needs 48 GB even with
 *   `enable_model_cpu_offload()`; the smaller flavour queues for hardware and
 *   then fails on memory.
 *
...
console.log(JSON.stringify({
  ...
  with_deps: DEPS,
  env: { PLATE: plate, MODE: mode, ...(seed ? { SEED: String(seed) } : {}) },
...
If the runner ignores \`env\`, prepend the assignments instead:
  ...
  os.environ["MODE"]  = ${JSON.stringify(mode)}${seed ? `\n  os.environ["SEED"]  = ${JSON.stringify(String(seed))}` : ''}
