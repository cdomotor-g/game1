...
each one's mark is). The same run writes the two pages that show them:
[`index.html`](index.html), which is the set, and [`print.html`](print.html),
which is the paper.
...
## Getting one onto paper

[`print.html`](print.html) — generated by the same tool as the sheets, so the
paper, the bleed and the cell on it are the sheets' own numbers rather than a
second copy that can disagree with them.

It asks **which ground and how many** before it puts anything on paper. That is
the whole reason it exists: a mini-map is A4 landscape and full bleed, so a page
that can only print the set prints eleven pages of solid colour, and what a table
wants is the ground it is fighting or building on — rarely more than two sheets.
`print.html?terrain=marsh` opens on that one, which is what every caption on the
index and every terrain in the explorer links to; a bare `print.html` opens on
one of each, and says how many pages that is before you press anything.

**A4 landscape and no other paper.** Not an omission: the cell *is* a world hex,
and a sheet scaled to fit some other size has cells that are not. The page sets
`@page` to the sheet's own 297 × 210 mm, previews at true size, and undoes its
own preview scale on `beforeprint` — the same belt and braces the card and map
print pages wear, for the same reason. Print at 100%, no "fit to page": the 3 mm
of bleed then runs off the edge of the paper, which is the only place bleed is
any use.
