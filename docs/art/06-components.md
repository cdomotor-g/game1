...
- **Face:** the plate, cropped to the footprint and clipped to the cut, with the tile's name in
  a band **hugging the lower-right edge**, type running parallel to it. The band was ruled
  corner to corner across the middle once; it was the longest band a hexagon can carry and it
  split every drawing into two unrelated halves. **No number of any kind**, on the same rule as
