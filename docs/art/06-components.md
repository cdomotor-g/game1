...
- **Face:** the plate, cropped to the footprint and clipped to the cut, with the tile's name
  in a band across the widest row's shoulder line — the height at which a pointy-top hex is
  still at full width. **No number of any kind**, on the same rule as the cards: everything
  that moves is counted on a board, and the mini-map's HOLDINGS panel already has the rows.
- **Back:** the placement side, and the reason the second side is worth printing. A building
  is laid back-up the round its work starts and stays there until the effort is paid — which
  is exactly when a player needs to know what ground it may stand on. So the back carries an
  engine-turned ground, the word (**SITE**, or **SOWN** for a field) and the world map's own
  terrain marks for where it is allowed. A back naming no ground means it stands anywhere.
