...
> **Note.** cards: 11 of 71 plates are GENERATED, not commissioned - Items and Tools, drawn by `node tools/draw-item.mjs` from the card's own data. They go through the same four steps as anything else and they are nobody's turn to draw: delete one and the tool puts it back.

> **Note.** cards: every plate clears the 945 px floor (a 80 mm card window at 300 dpi), and 37 of 71 are under the 1890 px this line would want for print — smallest is `TAL-01` at 1254 px. That is an aspiration, not a fault.
