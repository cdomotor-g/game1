...
![Homer's Odyssey mark](../art/icons/campaign-homers-odyssey.svg) **The mark.** A black ship under a square sail - the one thing every chapter of the poem has in it. Stroked on the 24-grid like the element marks, and built to survive at the size of a card code: a hull, a mast and a sail, three strokes, nothing finer. It is printed beside the card code, top right, of every card that belongs to the campaign - its own, and every character and monster it brings - so a table can pull them out of the free-play decks by the corner alone.
