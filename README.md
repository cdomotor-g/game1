...
| **S** | Strength | 1 | 0–14 | Set from the character card; read against whatever you fight |
...
useless for one lying in a recess with five others. So the board takes them over: one
place to look, one place to knock the tokens off, and the cards stay flat.

**Every board is identical**, because everything that differs between an orc and a
halfling is printed on the character card lying in the recess. A board that knew whose it
was would be five boards to print, five to keep in step, and four of them wrong for
whoever picked it up.

**0–14 is the game's ceiling, not just the board's.** `data/playerboard.json` lists every
number a token walks — health, strength, burden, damage, mana, on characters, monsters,
vehicles, items and peoples alike — and `node tools/validate-data.mjs` sweeps the lot
against the top rung. A fifteenth point of health fails the build rather than walking off
the top of a board somebody has already printed. That ceiling is why the mass scale was
halved: a hero carrying 28 kg cannot stand on a track that stops at 14, so the kilogram
got smaller rather than the board getting longer. Nothing changed about what fits in whose
hands.

**Strength settles fights.** It was already on every monster card as a threat rating; it
now shifts the number you need on an attack roll by the difference between your strength
and your opponent's — less yours, plus theirs. Equal strength still hits on 4+, a point of
advantage is worth exactly one pip, and it never adds dice, because dice are what weapons
and armour give you. Clamped to 2+ and 6+, so a dragon is terrifying rather than
arithmetic.
