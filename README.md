...
data/          the rules as data — the single source of truth, one file per system
  ...
  characters.json    the character deck — health, strength, defence, mana, gold
  quests.json        the quest deck — mini-quests and campaigns
  marketboard.json   the market board: identical price ladders, walked by commodity tokens
...
docs/          the published website AND the design documents — see Publishing
  ...
  markets/           GENERATED market board, from tools/build-market.mjs —
                     identical price ladders; a commodity token on a band IS
                     the price, and nothing on the sheet names a commodity
...
node tools/build-market.mjs    # regenerate the market board
...
round's phases printed from `rules.json`, and **six numbered tracks up the middle**:
...
is left under the heads. Add a seventh track and the columns narrow; nothing runs off the
paper, because nothing was ever positioned. Taking the border off — 18 mm of paper doing
nothing but making the middle narrower — is what paid for the sixth track, and the columns
came out *wider* with six than they had been with five.

## The market board

**[docs/markets/](https://cdomotor-g.github.io/game1/markets/)** — one A4 landscape sheet
of price ladders, generated by `tools/build-market.mjs` from
[`data/marketboard.json`](data/marketboard.json), the shapes in
[`data/components.json`](data/components.json) and the bands in
[`data/rules.json`](data/rules.json).

A town's price for a commodity is its base value times a band, and the band is the only
part that moves. So the board prints the bands — ×0.5, ×0.75, ×1, ×1.25, ×1.5, ×2 — and
**nothing else**: you say which commodity a line is about by standing that commodity's
hexagonal token on it. Where the token stands is the price; which token it is says what the
price is of. Look at the table and you can see what everything is worth.

**Every line is identical and no line is labelled.** A line is not the grain line until
somebody puts the grain token on it. One board serves any commodity, any town and any
table; print another sheet for another town's market; a sixty-seventh commodity reprints
nothing. Sixteen lines to a sheet, in two blocks of eight, and the ×1 column rules heavier
and carries a hairline hexagon in every line, so a board at setup is one straight column of
tokens.

Like the player board, none of it is placed by hand: a band cell is what the blocks leave,
and a line is exactly one commodity token tall — so a bigger token makes a *taller* line
and fewer of them, and never a token that overhangs its cell. The build fails rather than
printing one that does.
...
**It is a flat colour and a grid, and nothing else.** No render, no pattern, no hatch, no
drawn ground — so it needs no plate, no artist and no place in the mint queue. Drawn ground
competes with the pieces standing on it, and every one of these sheets is a sheet somebody
is standing pieces on. The colour is the one that terrain already prints in
