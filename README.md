...
docs/          the published website AND the design documents — see Publishing
  ...
  boards/            GENERATED player boards, one per playable people, from
                     tools/build-board.mjs — index.html shows them and prints
                     them one to a sheet of A4 landscape at true size
...
tools/
  ...
  build-icons.mjs    data/arcana.json → docs/art/icons/, the element marks
...
node tools/build-board.mjs     # regenerate the player boards
...
**What a card *is*** — its size, its 3.5 mm corner, its frame weights, where its bars
hang, what its deck's back looks like — is declared once in
...
per playable people, generated by `tools/build-board.mjs` from
...
each board comes off a sheet at true size.

A board is a place for the hero's card, four more for whatever else is in play, the
round's phases printed from `rules.json`, and **five numbered tracks up the middle**:

| | Track | A rung | Tops out | What walks it |
| --- | --- | --- | --- | --- |
...
**Why a board has tracks when the cards already have bars.** The bar convention hangs
harm off a card's left edge and capacity off its right — perfect for a card in the hand,
useless for one lying in a recess with four others. So the board takes them over: one
place to look, one place to knock the tokens off, and the cards stay flat. Four of the
five are bars that already exist; **Speed** is the board's own, and it is the number a
travelling party has been holding in its head every round since travel was added.
...
is left once the head and the seat have. Add a sixth track and the columns narrow; nothing
runs off the paper, because nothing was ever positioned. At A4 the two divisions land
within a tenth of a millimetre of each other, so the cells come out square and a 7 mm bar
token sits in the middle of one without touching a rule — an accident of the sheet size,
not a number anybody typed.

**Fifteen rungs is checked, not chosen.** It is the shortest ladder that covers every
printed value in the game: 13 health, 28 kg, 12 damage boxes, a 10-mana phylactery, 8
hexes for a rider on a road. Each track names the largest value it must cover *and* where
that number comes from, and `node tools/validate-data.mjs` recomputes it from the data —
so a sixteenth point of health fails the build instead of walking a token off the top of
a printed board.

**The five boards differ only in the player colour**, which appears in the border banding
and the maker's mark and nowhere else. The working surface stays neutral so a player's own
tokens read against it, and even in the banding the colour is a wash with the people's
hatch struck over it on the ink plate — the hatch is what identifies a player, the colour
is a convenience. Adding a sixth people to `data/peoples.json` prints a sixth board and
...
that knows what a burden bar is or why hills and marsh must never share a wash. The
