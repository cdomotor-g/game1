...
docs/          the published website AND the design documents — see Publishing
  ...
  CARD-MINT.md       how a card is minted: designer writes it, artist draws it,
                     and the queue that says whose turn it is
  ...
  art/               the visual style guide, see docs/art/README.md
    ...
    prompts/           commissioning briefs for peoples, monsters, characters,
                       vehicles, talismans and modifications; renders land in
                       art/renders/
    renders/           the accepted renders, one per prompt id — see its README
    icons/             GENERATED element marks, from tools/build-icons.mjs
    mint/QUEUE.md      GENERATED worklist: every card that is not a card yet
...
tools/
  ...
  mint-queue.mjs     which cards are waiting on a brief, a plate or a framing entry
...
A card only exists once four things do: an entry in `data/`, a brief in
`docs/art/prompts/`, a plate in `docs/art/renders/`, and a framing entry. Those are two
different jobs — a designer writes cards and cannot draw, an artist draws and does not
know what a burden bar is — so [`docs/CARD-MINT.md`](docs/CARD-MINT.md) sets out the
handover: one pull request per run, one comment thread per subject, the brief pasted in
full.

Which cards are waiting on what is **computed, not tracked**:

```bash
node tools/mint-queue.mjs      # → docs/art/mint/QUEUE.md
```

It reads the data, the briefs, the renders folder and the framing entries and works out
where every card has got to. There is no board to fall out of date, because there is no
board — add a card to `data/` and it appears in the queue the next time the tool runs.
