...
data/          the rules as data — the single source of truth, one file per system
  ...
  pricing.json       how a price is arrived at: the red, blue and green dice, the swing
                     ruler, and the three market-memory models a commodity can run under
  marketboard.json   the market board: identical lines of tally, memory and price ladder
...
docs/          the published website AND the design documents — see Publishing
  ...
  markets/           GENERATED market board, from tools/build-market.mjs —
                     identical lines of tally, memory and price ladder, over a
                     foot carrying the five dice and the swing ruler; a token on
                     a band IS the price, and nothing on the sheet names a
                     commodity
...
of market lines, generated by `tools/build-market.mjs` from
...
A town's price for a commodity is its base value times a band. What *moves* the band is a
roll and a memory: **two red dice are demand, two blue are supply, one green says how hard
the swing lands**, and every line carries a modifier from −3 to +3 for what that market has
already been through. So a line is three strips read left to right — a **tally** of what
the board is holding, the **memory** the tally moves, and the **price ladder** the
commodity's own hexagonal token walks. The foot of the sheet carries the five dice, the
elasticity strip and the seven-cell swing ruler that turns a roll into a number of bands.
Everything a Market phase needs is on one piece of paper.

**Every line is identical and no line is labelled.** A line is not the grain line until
somebody puts the grain token on it — and the token brings the rest with it, including the
mark in its corner saying which of the three market-memory models the strips are running
under: a **glut** that drowns when you flood it, a **hype** that runs after its own price,
a **depletion** that ratchets one way for the rest of the game. One board serves any
commodity, any town and any table; print another sheet for another town's market; a
sixty-seventh commodity reprints nothing.

Like the player board, none of it is placed by hand: a strip cell is a bar token wide, a
band cell is what the strips leave, and a line is exactly one commodity token tall — so a
wider memory track narrows the ladder and a bigger token makes a *taller* line and fewer of
them, and never a piece that overhangs its cell. The build fails rather than printing one
that does.
