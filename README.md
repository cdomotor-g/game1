...
data/          the rules as data — the single source of truth, one file per system
  ...
  expansions.json    which set a thing arrived with: an `expansion` tag on a record
                     files it under its own annex of the book. Empty until one exists
...
docs/          the published website AND the design documents — see Publishing
  ...
  rules/             the RULEBOOK CHAPTERS, hand-written, player-facing, and only
                     what a table needs to play; numbers are {{data.path}} tokens
                     the build resolves. See docs/rules/README.md
  review/            the ADVERSARIAL REVIEW of the rules - Addendum I of the book;
                     findings R-nn with a severity and a status, proposals P-nn
  book/              GENERATED printed book from rules/*.md, data/*.json,
                     design/*.md and review/*.md, via tools/build-book.mjs — Book I
                     the rules, then the reference tables, the illustrated
                     catalogue, one annex per campaign, the design notes, the
                     review; every section printable alone.
                     fonts/ are its OFL faces, committed so it sets the same
                     off disk as on the site
  ...
  design/            fifteen design documents, see docs/design/00-overview.md
                     14-annex.md is GENERATED by tools/build-annex.mjs. The
                     reasoning behind the rules; the rules themselves are rules/
...
**[docs/book/](https://cdomotor-g.github.io/game1/book/)** — *The Almanac*, the printed
book, generated by `tools/build-book.mjs` and committed. It is six things in one page,
each opening on a title page that carries a plate from the catalogue, and each printable
on its own from the **Print…** picker in the bar or the button on its title page:

| Part | What it is | Where it comes from |
| --- | --- | --- |
| **Book I — The Rules** | Only what a table needs to play. No history, no reasoning | `docs/rules/*.md`, hand-written; every constant a `{{data.path}}` token |
| **Annex I — The Reference Tables** | Every number in the game, as tables | `docs/design/14-annex.md`, itself generated from the data |
| **Annex II — The Catalogue** | Every thing in the base game with its plate whole: characters, monsters, buildings and vehicles at half a page each, everything else at a quarter | `data/*.json` and `docs/art/renders/` |
| **Annex III… — Campaign I, II…** | Everything a table needs to play a campaign: the rules of it, the board, its cast and monsters with their plates, every chapter card in order, what it teaches | `data/campaigns.json`, `data/maps/`, and the records tagged with the campaign |
| **Expansion I, II…** | The same catalogue for anything an expansion adds | records tagged `expansion`, resolved against `data/expansions.json` |
...
`node tools/book-proof.mjs [section…]` prints it to a PDF through a headless Chromium so
it can be looked at before it is shipped.
